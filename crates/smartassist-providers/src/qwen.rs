...
const DEFAULT_API_BASE: &str = "https://dashscope.aliyuncs.com/compatible-mode/v1";
...
impl QwenProvider {
    /// Create a new Qwen provider with an API key.
...
impl Provider for QwenProvider {
    ...
    async fn chat(
        ...
        debug!("Sending request to Qwen/DashScope API (model: {})", model);
    ...
    async fn chat_stream(
        ...
        // Streaming not yet implemented for Qwen
    ...
    async fn count_tokens(&self, _model: &str, _messages: &[Message]) -> Result<TokenCount> {
        Err(ProviderError::internal(
            "Token counting not supported by Qwen",
        ))
    }
...
mod tests {
    ...
    fn test_provider_creation() {
        let provider = QwenProvider::new("test-key").unwrap();
        assert_eq!(provider.name(), "qwen");
    }

    #[test]
    fn test_empty_key_rejected() {
        assert!(QwenProvider::new("").is_err());
    }
}