...
const DEFAULT_API_BASE: &str = "https://api.moonshot.cn/v1";
...
impl MoonshotProvider {
    /// Create a new Moonshot provider with an API key.
    ...
    /// Convert messages to Moonshot API format.
...
impl Provider for MoonshotProvider {
    ...
    async fn chat(
        ...
        debug!("Sending request to Moonshot API (model: {})", model);
    ...
    async fn chat_stream(
        ...
        // Streaming not yet implemented for Moonshot
    ...
    async fn count_tokens(&self, _model: &str, _messages: &[Message]) -> Result<TokenCount> {
        Err(ProviderError::internal(
            "Token counting not supported by Moonshot",
        ))
    }
...
mod tests {
    ...
    fn test_empty_key_rejected() {
        assert!(MoonshotProvider::new("").is_err());
    }
}